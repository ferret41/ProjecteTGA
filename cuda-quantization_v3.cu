...
__global__ void matrix_reduction_color_2(Color new_means_2[], Color new_means[], int Size_row, int Size, int N_colors, int offset) {
    ...
    for (int j = 0; j < N_colors; ++j) {
        ...
        if (offset == 2)         shared[tid*N_colors + j] = new_means[id*N_colors + j].r;
        else if (offset == 1)    shared[tid*N_colors + j] = new_means[id*N_colors + j].g;
        else                     shared[tid*N_colors + j] = new_means[id*N_colors + j].b;
    }
